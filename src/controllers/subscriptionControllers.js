...
// const User = require('../models/userModels'); // Asegúrate de tener tu modelo de usuario
// const { getIO } = require('../sockets');
...
//     // Obtener el vendedor para extraer su collector_id (vinculado a su cuenta de Mercado Pago)
//     const vendedor = await User.findById(vendedorId);
//     if (!vendedor || !vendedor.mercadoPagoCollectorId) {
//       return res.status(400).json({
//         status: "error",
...
//       // Parámetros para split payment:
//       collector_id: vendedorMercadoPagoId,
//       application_fee: parseFloat(amount) * 0.10, // Retener el 10% como comisión
//     };
...
// // (Opcional) Endpoint para consultar el estado de suscripción
// // Esto te permite, en el frontend, saber si el usuario ya está suscrito a un vendedor.
// const subscriptionStatus = async (req, res) => {
//   try {
//     // Se espera que se envíe como query el vendedorId
//     const { vendedorId } = req.query;
//     // Aquí deberás consultar tu base de datos para verificar si el usuario (req.user.id) tiene una suscripción activa al vendedorId.
//     // Esto dependerá de cómo hayas modelado las suscripciones.
//     // Por ejemplo:
//     const isSubscribed = false; // Supongamos que realizas la consulta y determinas este valor
